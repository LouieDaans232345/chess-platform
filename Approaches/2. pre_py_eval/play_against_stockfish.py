...
class ChessGame:
    def __init__(self, master):
        self.master = master
        self.master.title("Play Against Stockfish")
        ...
        self.stockfish.set_skill_level(10)  # Adjust skill level (0-20)
    ...
    def create_widgets(self):
        ...
        self.status_label = tk.Label(right_frame, text="Your move (White)", bg='#2c3e50', fg='white')
        ...
        tk.Button(button_frame, text="Resign", command=self.resign).pack(side=tk.LEFT, padx=5)
    ...
    def make_move(self, move):
        ...
        if not self.board.is_game_over():
            ...
            self.master.update()
            self.stockfish.set_fen_position(self.board.fen())
            stockfish_move = self.stockfish.get_best_move_time(1000)  # Limit thinking time to 1 second
            if stockfish_move:
                chess_move = chess.Move.from_uci(stockfish_move)
                san_move = self.board.san(chess_move)
                self.board.push(chess_move)
                self.move_list.insert(tk.END, f"{self.board.fullmove_number-1}... {san_move}")
                self.status_label.config(text="Your move (White)")
            else:
                self.status_label.config(text="Stockfish couldn't find a move")
            self.update_display()
    ...
    def save_game(self):
        ...
        filename = f"human_vs_stockfish_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pgn"
